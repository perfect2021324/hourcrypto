...
let appreciationForVote = async (jobId) => {
  ...
  let voterList = []
  if (appreciation > 0) voterList = job.votes.up
  if (appreciation < 0) voterList = job.votes.down
  if (appreciation == 0) voterList = [...job.votes.up, ...job.votes.down]
...
let settleJob = async (session, jobId) => {
  ...
  if (appreciation < 0) voterList = job.votes.down
  if (appreciation == 0) voterList = [...job.votes.up, ...job.votes.down]
    if (voterList == job.votes.up) {trans 
  }
    if (!trans?._id) {
