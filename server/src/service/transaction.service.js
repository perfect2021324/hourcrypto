...
let appreciationForVote = async (jobId) => {
  ...
  const createdBy = "SYSTEM"
  const amountWhenEqual = 1.5;

  for (const voter of mostvoter) {
    if (!voter?._id) continue
    const amount = appreciation === 0 ? amountWhenEqual : 2
    let trans
  ...
  for (const voter of lessVoters) {
    if (!voter?._id) continue
    const amount = appreciation === 0 ? amountWhenEqual : 1
    let trans
    try {
      trans = await makeTransaction({
        type: "bonus",
  ...
  for (const voter of job.votes.flag) {
    if (!voter?._id) continue
    const amount = job.votes.flag.length > job.votes.up.length ? 3 : job.votes.flag.length === job.votes.up.length ? amountWhenEqual : 0.1;
    let trans
    try {
      trans = await makeTransaction({
        type: "bonus",
...
let settleJob = async (session, jobId) => {
  try {
    ...
    // if appreciation > 0 and no one flag the job
    if (appreciation >= 0 && job.votes.flag.length === 0) {
